...
- **Суддівська панель**: touch-first інтерфейс `/judge` для таймера, дауну, періоду, рахунку й командних тайм-аутів. Кожна дія точково оновлює лише вибраний матч, а відкрита адмінка підтягує ці зміни кожні 5 секунд, якщо в ній немає незбережених правок.
- **Безпека**:
...
Суддя входить один раз через `POST /api/judge/verify` і отримує 18-годинний токен, який зберігається на пристрої без збереження пароля. Панель надсилає `PATCH /api/tournament` з дозволеними полями конкретного матчу: рахунок, ігровий час, даун, період, статус, використані тайм-аути й остання дія. Оптимістична версія турніру не дозволяє адмінці випадково перезаписати новіші суддівські дані.

Тривалість половини задається в адмінці у розділі «Турнір і тексти» та зберігається як `settings.halfDurationMinutes`. Значення використовується для нових матчів і кнопки скидання суддівського таймера; якщо налаштування ще не збережене, діє значення 20 хвилин. У матчі `timeouts.1` і `timeouts.2` окремо зберігають кількість використаних тайм-аутів господарями та гостями; ліміт — 2 на команду в кожній половині.
...
Точково оновлює один матч. Потребує `Authorization: Bearer <judge-token>` і тіло `{ "matchId": "...", "patch": { ... } }`. Дозволено синхронізувати рахунок, таймер, даун, період, статус, останню дію та `timeouts`. Зміни інших матчів, команд і текстів турніру не перезаписуються.
