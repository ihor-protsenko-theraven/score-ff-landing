...
import { handleAdminVerification, handleTournamentRequest } from './api.js';
...
export function createApp({ dataFile, publicDir, adminPassword, store = createFileTournamentStore(dataFile) }) {
  return createServer(async (request, response) => {
    const url = new URL(request.url, 'http://localhost');

    if (url.pathname === '/api/tournament') {
      await handleTournamentRequest(request, response, { store, adminPassword });
      return;
    }
